/*
 * @FilePath: lfp_socket_ctrl.c
 * @Description: Socket communication control main entrance.
 * @Author: wytaitaislee
 * @Date: 2021-08-27 23:29:52
 * @LastEditTime: 2022-03-08 22:48:43
...
LFP_INT32 lfp_socket_server_manage_node_recycle(LFP_VOID) {
  ...
  for (uiCnt = 0; uiCnt < LFP_NELEMENTS(pSocketServerManage->pSocketDesc);
       uiCnt++) {
    if (LFP_INVALID_SOCKET !=
        LFP_SOCKET_THIS_SERVER_DESC(pSocketServerManage, uiCnt)->iSockFd) {
      continue;
    }
    iExitAliveTime =
 ...
 * *pSocketCtrl)
 * @brief 	  socket communication entry init, including receiving/sending
 * data description
 * @param[in]  LFP_NULL
...
LFP_INT32 lfp_socket_create_subtask(LFP_INT32 iSocket) {
  ...
  for (uiCnt = 0; uiCnt < LFP_NELEMENTS(pSocketServerManage->pSocketDesc);
    ...
    if (!LFP_SOCKET_THIS_SERVER_DESC(pSocketServerManage, uiCnt)) {
      iRet = lfp_socket_desc_entry_init(
          &LFP_SOCKET_THIS_SERVER_DESC(pSocketServerManage, uiCnt), iSocket);
      break;
    }
  }
  if (uiCnt >= LFP_NELEMENTS(pSocketServerManage->pSocketDesc) ||
      LFP_OK != iRet) {
...
LFP_INT32 lfp_socket_recv_data(LFP_SOCKET_DESC_T *pDesc) {
  ...
  while (iRecvLen < iNeedRecvLen) {
    ...
    if (iRecvLen == sizeof(LFP_SOCKET_THIS_RECV_HEAD(pDesc)) &&
        iNeedRecvLen == sizeof(LFP_SOCKET_THIS_RECV_HEAD(pDesc))) {
      LFP_SOCKET_CTRL_ERR("iDataLen = %d, isocketFd = %d--------\n",
                          LFP_SOCKET_THIS_RECV_HEAD(pDesc).iDataLen,
                          pDesc->iSockFd);
      memcpy(&LFP_SOCKET_THIS_RECV_HEAD(pDesc),
             LFP_SOCKET_THIS_RECV_BODY(pDesc),
             sizeof(LFP_SOCKET_THIS_RECV_HEAD(pDesc)));
      iNeedRecvLen = LFP_MIN(LFP_SOCKET_THIS_RECV_HEAD(pDesc).iDataLen,
                             LFP_SOCKET_THIS_RECV_HEAD(pDesc).iMaxDataLen);
    }
  }
  LFP_SOCKET_CTRL_CRIT("%s, length : %d\n",
                       LFP_SOCKET_THIS_RECV_BODY(pDesc) +
                           sizeof(LFP_SOCKET_THIS_RECV_HEAD(pDesc)),
                       strlen(LFP_SOCKET_THIS_RECV_BODY(pDesc) +
                              sizeof(LFP_SOCKET_THIS_RECV_HEAD(pDesc))));
  return LFP_OK;
}
...
LFP_INT32 lfp_socket_send_data_ctrl(LFP_SOCKET_DESC_T *pDesc) {
  ...
  while (1) {
    if (LFP_OK !=
        lfp_socket_package_entry_init(&pDesc->struSocketCtrl.struSendCtrl)) {
      iRet = LFP_ERR;
    ...
    while (!fgets(aInputBuf, sizeof(aInputBuf) - iOffset, LFP_STDIN)) {
      if (0 == strncmp(aInputBuf, LFP_SOCKET_EXIT_REQUEST_KEYS,
                       LFP_MIN(strlen(aInputBuf),
                               strlen(LFP_SOCKET_EXIT_REQUEST_KEYS)))) {
    ...
    memcpy(LFP_SOCKET_THIS_SEND_BODY(pDesc), &LFP_SOCKET_THIS_SEND_HEAD(pDesc),
           sizeof(LFP_SOCKET_THIS_SEND_HEAD(pDesc)));
    if (LFP_OK !=
        lfp_socket_send_data(pDesc->iSockFd, LFP_SOCKET_THIS_SEND_BODY(pDesc),
