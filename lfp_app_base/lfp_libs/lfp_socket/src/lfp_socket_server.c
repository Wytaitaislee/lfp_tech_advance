/*
 * @FilePath: lfp_socket_server.c
 * @Description: Socket communication server implementation.
 * @Author: wytaitaislee
 * @Date: 2021-08-27 23:29:52
 * @LastEditTime: 2022-03-08 22:49:00
...
LFP_INT32 lfp_socket_proc(LFP_VOID* pServerArgs) {
  ...
  /*
          ...
          {
                  LFP_SOCKET_SERVER_ERR("lfp_socket_fcntl[socket = %d]
     err[%d][%s]\n", pServerDesc->iSockFd, errno ,strerror(errno)); iRet =
  ...
  */
  ...
  while (1) {
    ...
    if (LFP_PTHREAD_ESRCH == lfp_pthread_kill(struSendTid, 0) ||
        LFP_PTHREAD_ESRCH == lfp_pthread_kill(struRecvTid, 0)) {
...
LFP_INT32 lfp_socket_server_task(LFP_VOID* pArgs) {
  ...
  iSockServer =
      lfp_socket_create(LFP_AF_INET, LFP_SOCK_STREAM, LFP_IPPROTO_TCP);
  ...
  if (LFP_OK != lfp_socket_bind(iSockServer, (LFP_SOCK_ADDR_T*)&struSockAddrIn,
                                sizeof(struSockAddrIn))) {
  ...
  while (1) {
    ...
    iSockNew = lfp_socket_accept(
        iSockServer, (LFP_SOCK_ADDR_T*)&struSockAddrNew, &struSockLen);
